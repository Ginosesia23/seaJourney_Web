...
export function CertificatesPageHeader({
  title = 'Certificates',
  description,
  actions,
}: {
...
export function CertificatesStatTiles({
  items,
}: {
  items: Array<{
    label: string;
...
export function CertificatesEmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 py-12 text-center">
      <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-md border border-border bg-muted/40">
...
export function CertificatesLoadingState() {
  return (
    <div className="flex items-center justify-center gap-2 py-12">
      <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />
      <span className="text-xs text-muted-foreground">Loading certificates…</span>
    </div>
  );
}
