...
export async function loadFeatureFlagState(opts?: {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select(
      'key, enabled, note, updated_at, updated_by, last_enabled_at, last_disabled_at',
    );

  if (error) {
    // Older schema without toggle timestamp columns — retry without them.
    const fallback = await supabaseAdmin
      .from('platform_feature_flags')
      .select('key, enabled, note, updated_at, updated_by');
    if (fallback.error) {
      console.warn(
        '[feature-flags] load failed, using defaults:',
        fallback.error.message,
      );
      const map = resolveFeatureEnabledMap([]);
      cache = { at: Date.now(), map, rows: [] };
      return cache;
    }
    const rows = (fallback.data || []).map((r) => ({
      ...
      last_enabled_at: null as string | null,
      last_disabled_at: null as string | null,
    }));
    cache = {
      at: Date.now(),
...
export async function setFeatureFlagEnabled(opts: {
  ...
  if (error) {
    // Schema without timestamp columns — retry minimal payload.
    if (
