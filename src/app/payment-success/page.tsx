    ...
    const verifyPayment = async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          try {
            // Update user profile in Supabase with subscription information
            // This will create the user if they don't exist
            await updateUserProfile(supabase, user.id, {
              subscriptionStatus: 'active',
              subscriptionTier: tier,
              email: user.email, // Pass email in case user needs to be created
            });
            ...
            setError(`Failed to update subscription: ${supabaseError?.message || 'Unknown error'}`);
            toast({
              title: 'Update Failed',
              description: supabaseError?.message || 'Payment verified but failed to update subscription. Please contact support.',
              variant: 'destructive',
            });
          }
        } else {
      ...
      <div className="dark animated-gradient-background flex min-h-screen flex-col items-center justify-center px-4">
        ...
        <Card className="w-full max-w-md border-destructive/50 bg-black/20 backdrop-blur-sm">
          <CardHeader className="text-center">
            <CardTitle className="font-headline text-2xl text-destructive">Payment Verification Failed</CardTitle>
        ...
        <div className="relative w-full max-w-md p-1 border border-primary/20 rounded-xl bg-black/20 backdrop-blur-sm">
          ...
          <Card className="w-full border-none bg-transparent text-card-foreground shadow-none rounded-xl">
            <CardHeader className="text-center">
              ...
              <CardTitle className="font-headline text-2xl">Payment Successful!</CardTitle>
              <CardDescription>
                Your subscription has been activated. You now have full access to SeaJourney.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
