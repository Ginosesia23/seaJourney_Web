...
export default function InboxPage() {
  ...
  // Helper function to get crew member name from testimonial
  const getCrewMemberName = (testimonial: typeof testimonials[0]) => {
    const user = testimonial.user as any;
  ...
  const isGeneratedByVesselManager = (testimonial: typeof testimonials[0]): boolean => {
      ...
      {isVesselAccount && inboxView === 'sent' ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          {(() => {
            ...
            // Removed debug log for vessel accounts
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex-1">
                                        <div className="flex items-center gap-2 flex-wrap">
                                          <span className="font-semibold">{getCrewMemberName(testimonial)}</span>
                                          {isGeneratedByVesselManager(testimonial) && (
                                            <Badge variant="outline" className="border-blue-500 text-blue-700 bg-blue-50 dark:bg-blue-950/30 dark:text-blue-400 text-xs">
                                              Generated by Vessel
                                            </Badge>
                                          )}
                                        </div>
                          ...
                          <div className="text-xs text-yellow-800 dark:text-yellow-200 space-y-1 max-h-32 overflow-y-auto">
                            {comparisonData.discrepancies.slice(0, 5).map((day: any, idx: number) => (
                              <div key={idx} className="flex items-center gap-2">
                                ...
                                <span>Your log: <strong>{day.crewState === 'underway' ? 'At Sea' : day.crewState === 'at-anchor' ? 'At Anchor' : day.crewState === 'in-port' ? 'Moored' : day.crewState === 'in-yard' ? 'In Yard' : day.crewState}</strong></span>
                                <span>→</span>
                                <span>Vessel log: <strong>{day.vesselState === 'underway' ? 'At Sea' : day.vesselState === 'at-anchor' ? 'At Anchor' : day.vesselState === 'in-port' ? 'Moored' : day.vesselState === 'in-yard' ? 'In Yard' : day.vesselState}</strong></span>
                              </div>
                            ))}
