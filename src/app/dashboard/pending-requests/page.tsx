...
export default function PendingRequestsPage() {
  ...
  const role = String(
    (userProfile as { role?: string } | null)?.role || userProfile?.role || '',
  ).toLowerCase();
  ...
  }, [user?.id, isVessel, activeVesselId, supabase]);
  ...
  if (userProfile && !isVessel) {
    return (
      <div className="mx-auto max-w-2xl py-16 text-center">
        <h1 className="text-xl font-semibold">Vessel accounts only</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          This page lists testimonials and sea-time access you have sent from a vessel account.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-end justify-between gap-4 border-b">
        ...
        >
          <SelectTrigger className="mb-1.5 h-8 w-[9.5rem] border-0 bg-transparent px-2 text-xs shadow-none hover:bg-muted/50">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All types</SelectItem>
            <SelectItem value="testimonial">Testimonials</SelectItem>
            <SelectItem value="access">Sea-time access</SelectItem>
          </SelectContent>
        </Select>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              <TableHead className="h-8 px-2 py-1.5 text-xs">Crew</TableHead>
              <TableHead className="h-8 px-2 py-1.5 text-xs">Type</TableHead>
              <TableHead className="h-8 px-2 py-1.5 text-xs">Period</TableHead>
              <TableHead className="h-8 px-2 py-1.5 text-xs">Days</TableHead>
              <TableHead className="h-8 px-2 py-1.5 text-xs">Captain</TableHead>
              <TableHead className="h-8 px-2 py-1.5 text-xs">Status</TableHead>
              <TableHead className="h-8 px-2 py-1.5 text-right text-xs">Sent</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {visible.map((item) => (
              ...
              >
                ...
                <TableCell className="max-w-[180px] truncate px-2 py-1.5 text-xs tabular-nums">
                  {item.period}
                </TableCell>
                ...
                <TableCell className="whitespace-nowrap px-2 py-1.5 text-right text-xs tabular-nums text-muted-foreground">
